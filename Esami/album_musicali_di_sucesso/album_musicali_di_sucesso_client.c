...
int main(int argc, char const *argv[])
{
    ...
    {
        // USERNAME
        printf("\n Inserisci username, 'fine' per terminare \n");
        scanf("%s", &insUtente);

        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
            fputs("Errore write all", stderr);
            exit(EXIT_FAILURE);
        }
        err = write(sd, "\n", 1);
        if (err < 0)
        ...
        // NOME ARTiSTA
        printf("\n Inserisci Nome artista, 'fine' per terminare \n");
        scanf("%s", &insUtente);
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
        ...
        // COGNOME ARTOSTA
        printf("\n Inserisci Cognome artista, 'fine' per terminare \n");
        scanf("%s", &insUtente);
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
