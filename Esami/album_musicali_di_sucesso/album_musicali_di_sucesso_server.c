...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if (pid00 == 0)
        {
            ...
            while (1)
            {
                ...
                // lettura COGNOME_CANTANTE
                memset(str4, 0, sizeof(str4));
                str4_len = sizeof(str4) - 1;
                err = rxb_readline(&rxb, ns, str4, &str4_len);
                if (err < 0)
                {
                    perror("rxb_readline");
                    exit(EXIT_FAILURE);
                ...
                if (strcmp(str1, "fine") != 0 && strcmp(str2, "fine") != 0 && strcmp(str3, "fine") != 0 && strcmp(str4, "fine") != 0 && autorizza(str1, str2) == 1)
                {
                    ...
                    { // PRIMO FIGLIO (SORT)
                        ...
                        execlp("sort", "sort", "-n", "-r", "music/album_database.txt", NULL);
                        perror("sort");
                        exit(EXIT_FAILURE);
                    }
                    ...
                    if (pid2 == 0)
                    {
                        ...
                        char buffy[5000];
                        snprintf(buffy, sizeof(buffy), "%s %s", str3, str4);
                        execlp("grep", "grep", buffy, NULL);
                        perror("grep");
                        exit(EXIT_FAILURE);
                    }
                    close(pip1[0]);
                    ...
                    if (err < 0)
                    {
                        perror("Errore write_all di fin");
                        exit(EXIT_FAILURE);
                    }
                    err = write(ns, "\n", 1);
                    if (err < 0)
                ...
                {
                    puts("Comunicazione terminata su richiesta del client o client non autorizzato");
                    rxb_destroy(&rxb);
                    exit(0);
                }
            }
    ...
    close(ns); // non previsto che esca
    close(sd);
    return -1;
}
