...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if (pid00 == 0)
        { // processo figlio capo gestione connessione
            ...
            while (1)
            {
                ...
                if (strcmp(str1, "fine") != 0 && strcmp(str2, "fine") != 0 && strcmp(str3, "fine") != 0)
                {
                    ...
                    if (pid1 == 0)
                    { // primo figlio (grep nome giocatore)
                        close(pip1[0]);
                        close(ns);
                        ...
                        char filename[5000];
                        snprintf(filename, sizeof(filename), "6_nazioni/%s.txt", str2);
                        execlp("grep", "grep",str1,filename, NULL);
                        perror("grep");
                        exit(EXIT_FAILURE);
                    }
                    int pid2 = fork();
