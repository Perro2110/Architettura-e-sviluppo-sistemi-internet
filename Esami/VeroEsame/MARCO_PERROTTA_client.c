...
// gcc -g -o ./visualizza_interventi_onerosi MARCO_PERROTTA_client.c rxb.c rxb.h utils.c utils.h
...
int main(int argc, char const *argv[])
{
    ...
    {
        /** Nome Cliente */
        printf("\nInserire nome cliente di interesse, 'fine' per terminare \n");
        scanf("%s", &insUtente);
        ...
        /** Numero N */
        printf("\nInserire numero N di interesse, 'fine' per terminare \n");
        scanf("%s", &insUtente);
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
        ...
        for(;;)
        {
            ...
            {
                fprintf(stderr,"Risposta non in vformato valido UTF-8");
                close(sd);
                exit(EXIT_FAILURE);
            }
#endif
