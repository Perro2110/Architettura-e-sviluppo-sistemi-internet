...
// execlp("sort", "sort", "-n", "-r", "-k", "2", "-t", ",", NULL)  
//                         ==> sorta seguendo campo k-esimo guardando delimitatore "-t ,"
        ...
        { // Processo figlio per gestire il client
            ...
            while (1)
            {
                ...
                if (strcmp(str1, "fine") != 0 && strcmp(str2, "fine") != 0 && strcmp(str3, "fine") != 0 && autorizza(str1, str2))
                {
                    ...
                    { // primo figlio (sort)
                        char filename[5000];
                        snprintf(filename, sizeof(filename), "%s.txt", str3);
                        // puts(filename);
                        close(pip1[0]);
                        close(pip2[0]);
                        close(pip2[1]);
                    ...
                    write(ns, "\n", 1);
                    write_all(ns, "==fin==", sizeof("==fine=="));
                    write(ns, "\n", 1);
                }
                else
                {
                    printf("Comunicazione negata o terminata\n");
                    rxb_destroy(&rxb);
                    exit(0);
                }
            }
