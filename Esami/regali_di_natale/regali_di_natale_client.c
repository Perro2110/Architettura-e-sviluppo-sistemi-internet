...
int main(int argc, char const *argv[])
{
    ...
    if (argc < 3)
    {
        fputs("Errore num. argomenti", stderr);
        exit(EXIT_FAILURE);
    }
    ...
    while (1)
    {
        ...
        if (strcmp(insUtente, "fine") == 0)
            return 1;

        /***********************/
        /*****  PASSWORD : ****/
        /*********************/
        printf("\ninserisci PASSWORD, 'fine' per terminare\n");
        scanf("%s", &insUtente);
        /* mando e scrivo string in formato UTF-8 */
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        ...
        /*****  CATEGORIA : ****/
        /**********************/
        printf("\ninserisci CATEGORIA giocatolo, 'fine' per terminare\n");
        scanf("%s", &insUtente);
        /* mando e scrivo string in formato UTF-8 */
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
