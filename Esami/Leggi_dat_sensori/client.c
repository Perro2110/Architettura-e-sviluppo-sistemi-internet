...
int main(int argc, char const *argv[])
{
    ...
    {
        /** Data di interesse */
        printf("\n Inserire data di interesse, 'fine' per terminare \n");
        scanf("%s", insUtente);
        ...
        /** Tipo di sensore di interesse */
        printf("Inserire tipo di sensore di interesse, 'fine' per terminare \n");
        scanf("%s", insUtente);
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
        ...
        /** Nome della stanza di interesse */
        printf("Inserire nome della stanza di interesse, 'fine' per terminare \n");
        scanf("%s", insUtente);
        err = write_all(sd, insUtente, strlen(insUtente));
        if (err < 0)
        {
        ...
        /** RICEZZIONE RISPOSTA DAL SERVER */
        for (;;)
        {
            ...
            {
                fprintf(stderr, "Risposta non è un valido UTF-8");
                close(sd);
                exit(EXIT_FAILURE);
            }
#endif
